...
**35 figures exist**, covering 34 of the 46 units that need one; unit 00 has two, and the
capstone needs none. **12 units still have no figure.**

**13 example documents** ship, with their media, all downloadable.
...
|---|---|---|---|
| 1 — authoring interactive scores | 00-18, P1-P4 (23) | done | 17 of 22 |
...
### 1. Two structural figures, now scripted

`p3-01` and `p4-01`. `mkscore.py` learned transitions on 2026-08-18, from two shapes Edu
drew by hand, so these need no interaction: write a builder, generate, capture. `lesson_17()`
is the worked example, and `checks/17-loops-and-out-of-time.md` has the JSON shape and the
two settings an endless document needs.
