...
Both devices are declared before anything is drawn, which is the order this course keeps insisting on. The output device is expanded on the left, and its shape is the thing to read: sixteen channels, each carrying `on`, `off`, `control`, `program`, and `pitchbend`. That is what addressing a channel rather than a device means in practice, and it is an ordinary address tree, no different in kind from the OSC one above it. The inspector says the rest: this piano roll writes to channel 1, its grid covers notes 60 to 71, and its output goes to `MIDI Out`. Dwell on that range, because it is this lesson's warning made concrete. The roll speaks in note numbers, and 60 to 71 is exactly one octave upward from middle C.
...
>
> **The address tree is an option, and only on the output.** `Create whole tree`, in the add-device dialog, is what builds those sixteen channels and their addresses. It is offered for `MIDI Output` and greyed out for `MIDI Input`, where *score* fills the tree from what actually arrives instead, which is why an input device with nothing plugged into it shows as a bare row. Note also that there are three MIDI protocols rather than two: `MIDI Input`, `MIDI Output`, and `MIDI Controller`, all under `Hardware`.
