...
4. **Now bound it structurally.** Undo the above, select the phrase, and put it inside a sub-scenario, so the loop is contained in one interval. Put a trigger on the *end of that interval*. Play, and fire the trigger: the loop and everything in it stops, and the score continues. This is the cleaner idiom when you want a definite exit.
