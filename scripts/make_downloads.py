...
def main() -> int:
    ...
    # The same inventory as data, for the list at the bottom of each lesson
    # (_includes/lesson_files.html). Documents first, then the media they use.
    lines = ["# Generated by scripts/make_downloads.py from library/learn/. Do not edit.", ""]
    for d in dirs:
        files = sorted((f for f in d.rglob("*") if f.is_file()),
                       key=lambda f: (f.suffix != ".score", f.name))
        lines.append(f"- slug: {d.name}")
        lines.append("  files:")
        for f in files:
            lines.append(f"    - name: {f.name}")
            lines.append(f"      path: {d.name}/{f.relative_to(d).as_posix()}")
            lines.append(f"      kind: {KIND.get(f.suffix, 'file')}")
            lines.append(f"      size: {human(f.stat().st_size)}")
    DATA.write_text("\n".join(lines) + "\n", encoding="utf8")
