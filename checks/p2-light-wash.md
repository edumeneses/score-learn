# Re-verification note: p2-light-wash

Pinned build: **ossia score 3.8.2**

## Figures

| ID | Content | Status |
|---|---|---|
| p2-01 | One interval, four automations, one per channel group | **done**, docs/learn/assets/p2/p2-01-light-wash.png |

See `checks/FIGURES-PENDING.md` for the consolidated list of figures that need
an interactive session, and why each one cannot be produced by the scripted
pipeline.

## Re-verify when the pinned version changes

- That a raw Art-Net device still exposes plain DMX channels and a whole-device message.
- The pattern syntax used to reach several channels with one curve.
- That 0-255 integer channels still behave as described when a curve's range is left at 0-1.

## Claims that depend on external sources

- Grounded in the reference documentation for this topic; see the 'Going further'
  links on the lesson page, which are the pages this lesson was written against.

- 2026-09-16 crop audit: Figure `p2-01`: crop extended to 1900 so the fourth automation slot (`lesson:/haze`) is complete. Re-rendered from the same raw.
